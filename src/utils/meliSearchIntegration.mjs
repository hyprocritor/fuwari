  ...
  const posts = await Promise.all(
    folders.map(async folderName => {
      ...
      const expressiveCode =
        contentElement?.querySelectorAll('.expressive-code')
      for (const block of expressiveCode ?? []) {
        block.remove()
      }
      const rlcContainers = contentElement?.querySelectorAll('.rlc-container')
      for (const container of rlcContainers ?? []) {
        container.remove()
      }
      ...
      for (const container of githubContainers ?? []) {
        container.remove()
      }
